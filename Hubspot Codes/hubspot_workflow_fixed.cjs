...
exports.main = async (event, callback) => {
  ...
  // 4) (Optionnel) Ajout à la liste statique (Lists v3, nécessite un listId v3 + contact IDs)
  // MODIFICATION: Suppression de la vérification de la liste qui nécessite crm.lists.read
  if (LIST_ID && guestIds.length) {
    try {
      // Tentative directe d'ajout sans vérification préalable
      ...
      if (!add.ok) {
        ...
        // Gestion spécifique des erreurs de scope ou de type de liste
        if (add.status === 403 && errorData.message && errorData.message.includes('scopes')) {
          errs.push(`add_to_list: missing_scope_permission - contact your admin to add crm.lists.read scope`);
