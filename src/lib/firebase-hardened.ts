...
function validateFirebaseConfig(): { isValid: boolean; error?: string } {
  ...
  if (process.env.NEXT_PUBLIC_ENABLE_FIREBASE !== 'true') {
    return { 
      isValid: false, 
      error: 'Firebase disabled - set NEXT_PUBLIC_ENABLE_FIREBASE=true to enable' 
    };
  }
  ...
  if (missing.length > 0) {
    return { 
      isValid: false, 
      error: `Missing required Firebase environment variables: ${missing.join(', ')}. Please add them to .env.local in the app root directory and restart the development server.` 
    };
  }
  
  // Check for placeholder values
  const placeholders = REQUIRED_FIREBASE_VARS.filter(key => {
 ...
 */
function initializeFirebase(): { app: FirebaseApp | undefined; db: Firestore | undefined; auth: Auth | undefined } {
  if (isInitialized) {
  ...
  if (!validation.isValid) {
    const error = validation.error || 'Unknown Firebase configuration error';
...
export async function loadVacationRequests(): Promise<VacationRequest[]> {
  try {
    ...
    const q = query(
      collection(firestore, VACATION_REQUESTS_COLLECTION),
      orderBy('createdAt', 'desc')
    );
...
export async function addVacationRequest(request: Omit<VacationRequest, 'id'>): Promise<string> {
  try {
    ...
    const docRef = await addDoc(collection(firestore!, VACATION_REQUESTS_COLLECTION), {
      ...request,
...
export async function updateVacationRequest(id: string, updates: Partial<VacationRequest>): Promise<void> {
  try {
    ...
    const docRef = doc(firestore, VACATION_REQUESTS_COLLECTION, id);
    await updateDoc(docRef, {
      ...updates,
...
export { firestore as db, auth, firebaseApp as app };
