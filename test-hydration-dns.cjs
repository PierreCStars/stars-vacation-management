...
async function testHydration() {
  ...
  // Listen for DNS errors
  const dnsErrors = [];
  page.on('requestfailed', request => {
    if (request.failure().errorText === 'net::ERR_NAME_NOT_RESOLVED') {
      dnsErrors.push(`${request.failure().errorText} ${request.url()}`);
    }
  });
